...
function mostrarPagina(pagina, datos, estadoFiltro) {
    ...
    mostrarPaginador(pagina, datosFiltrados.length, datosFiltrados);
}
...
function mostrarPaginador(paginaActual, totalFilas, datos) {    
    const paginador = document.getElementById("paginador");
    paginador.innerHTML = "";

    const numeroTotalDePaginas = Math.ceil(totalFilas / 40);
    ...
    botonAnterior.addEventListener("click", () => {
        if (paginaActual > 1) {
            mostrarPagina(paginaActual - 1, datos);
        }
    });
    ...
    for (let i = 1; i <= numeroTotalDePaginas; i++) {
        ...
        botonPagina.addEventListener("click", () => mostrarPagina(i, datos));
